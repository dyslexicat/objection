...
def ace_attorney_anim(config: List[Dict], output_filename: str = "output.mp4"):
    ...
    # only show log output for warnings
    out = ffmpeg.output(
        video,
        audio,
        output_filename,
...
def get_characters(most_common: List):
    ...
    #original code has this as > 0 which doesn't make sense?
...
def comments_to_scene(usernames: Dict, comments: List, characters: Dict, **kwargs):
    ...
    for comment in comments:
        ...
        # removed comment.score check
        # maybe we can add reaction check?
        if polarity < 0:
        ...
        for idx, chunk in enumerate(joined_sentences):
            character_block.append(
                {
                    "character": character,
                    "name": usernames[comment.author],
                    "text": chunk,
                    "objection": (polarity < 0) and idx == 0,
